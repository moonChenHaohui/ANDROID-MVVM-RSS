package com.moon.myreadapp.common.components.swipeback;
...
public class SwipeBackLayout extends FrameLayout {
     ...
     * 滚动生效百分比
     */
    private static float SCROLL_SIZE = .5f;
    ...
    public void attachToActivity(Activity activity) {
        mActivity = activity;
        isRootActivity = mActivity.isTaskRoot();
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (moveX - downX > mTouchSlop
                        && Math.abs((int) ev.getRawY() - downY) < mTouchSlop && !isRootActivity) {
                    return true;
                }
                break;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                tempX = moveX;
                if (moveX - downX > mTouchSlop
                        && Math.abs((int) event.getRawY() - downY) < mTouchSlop && moveX < viewWidth * ATREA_PERCETAGE ) {
                    isSilding = true;
                }
            ...
            case MotionEvent.ACTION_UP:
                isSilding = false;
                if (mContentView.getScrollX() <= -viewWidth * SCROLL_SIZE && !isRootActivity) {//RootActivity 需要scrollOrigin();
                    isFinish = true;
                    scrollRight();
                } else {
                    scrollOrigin();
